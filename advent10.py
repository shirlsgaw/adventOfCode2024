...
def is_valid_location(curr_x: int, curr_y: int, next_x: int, next_y: int,
  ...
  #if check_visited and (next_x, next_y) in visited:
  #  return False
...
def get_horizon(
    ...
    visited: set[tuple[int, int]], check_visited=True) -> Generator[tuple[int, int], None, None]:
  # North
  #print(f'. North of ({x}, {y})')
  next_y = y - 1
  next_x = x
  if is_valid_location(x, y, next_x, next_y, map, visited, check_visited):
    yield (next_x, next_y)
...
for (x, y) in find_trailheads(topo_map):
  ...
  while len(unexplored) > 0:
    ...
    #print(f'({curr_x}, {curr_y}): {topo_map[curr_y][curr_x]}')
    ...
    if topo_map[curr_y][curr_x] == '9':
      ...
      for hx, hy in get_horizon(curr_x, curr_y, topo_map, visited, check_visited=False):
        unexplored.append((hx, hy))
        #print(f'...Horizon ({curr_x}, {curr_y}): {topo_map[curr_y][curr_x]} -> ({hx}, {hy})')
  print(f'Trailhead: ({x}, {y}): {count}')
  sum += count
